...
public class SmsToCmd extends SmsCmdBase {
    ...
    public static void Smsto(Chat chat, String message) {
        ...
        if (hasSecArgs(message)) {
            ...
            if (Tools.isNumeric(findStr)) {
                ...
                sendMessageAndUpdateView(chat, "Send SMS To : " + findStr + " ( Number : "
                        + findStr + " Body : " + body + " )" + " Done");
            }
            // 不全为数字则去查询号码是否为联系人号码
            else {
    ...
    public static void SelPhoneNumberDone(String addressNumber) {
        ...
        // 考虑去动态注册广播接收器来判断短信发送的状态
        sendMessageAndUpdateView(chat, "Send SMS To : " + contactName + " ( Number : "
                + addressNumber + " Body : " + body + " )" + " Done");
    }
    ...
    static void sendSMS(String findStr) {
        ...
        if (NumberOfPhoneNumber != 0) {
            ...
            if (NumberOfPhoneNumber != 1) {
                sendMessageAndUpdateView(chat, SelCmd.createChoices(
                        Contact.getContactNumberByName(contactName), "Select PhoneNumbers"));
            }
            // 号码唯一则直接发送
            else {

                String addressNumber = Contact.getContactNumberByName(contactName).get(0)
                        .toString();
