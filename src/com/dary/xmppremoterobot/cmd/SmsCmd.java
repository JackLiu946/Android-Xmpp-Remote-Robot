...
public class SmsCmd extends SmsCmdBase {
    ...
    public static void Sms(Chat chat, String message) {
        ...
        // 不带参数,则返回短信记录.
        if (!hasArgs(message)) {
            int smsCommandDisplayItemsNumber = Integer.parseInt(PreferenceManager
                    .getDefaultSharedPreferences(MyApp.getContext()).getString(
            ...
            MyApp.getContext().getContentResolver()
                    .update(uri, cvalues, " address='" + lastAddress + "'", null);
            ...
            if (getArgsCaseSensitive(message).length() == 0) {
                ...
                sendMessageAndUpdateView(chat,
                        "Send SMS " + "( Number : " + Contact.getContactNameByNumber(lastAddress)
                                + " Body : " + getArgsCaseSensitive(message) + " )" + "Done");
            }
        }
    }
}
