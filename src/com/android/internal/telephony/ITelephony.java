...
public interface ITelephony {
	 ...
	 * 
	 * @param number
	 *            the number to be dialed. If null, this would display the Dialer screen with no number pre-filled.
	 */
	 ...
	 * 
	 * If there's already a current active call, that call will be automatically put on hold. If both lines are currently in use, the current active call will be ended.
	 * 
	 * TODO: provide a flag to let the caller specify what policy to use if both lines are in use. (The current behavior is hardwired to "answer incoming, end ongoing", which is how the CALL button is specced to behave.)
	 * 
	 * TODO: this should be a oneway call (especially since it's called directly from the key queue thread).
	 */
	 ...
	 * 
	 * It's safe to call this if the ringer has already been silenced, or even if there's no incoming call. (If so, this method will do nothing.)
	 * 
	 * TODO: this should be a oneway call too (see above). (Actually *all* the methods here that return void can probably be oneway.)
	 */
	void silenceRinger();
